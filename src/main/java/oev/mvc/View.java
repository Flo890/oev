package oev.mvc;

import java.util.Observer;

import java.util.Observable;

import javax.swing.*;
import java.awt.*;
import java.awt.event.MouseEvent;
import java.awt.event.MouseListener;
import java.io.IOException;
import java.net.URI;
import java.net.URISyntaxException;


public class View extends JFrame implements Observer{
	
	Controller controller;

	JTextField srcPath, resPath, amountFrames, amountNachziehen, startFrameField;
	JComboBox fktBox;
	ButtonGroup modeGroup;
	JLabel lastAction1, lastAction2, lastAction3;
	JProgressBar progressBar;
	
	public View(Controller c){
		controller = c;
		setTitle("OEV 1.3.2");//TODO read version from gradle file / gradle.properties
        setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
        
    	JMenuBar menuLeiste = new JMenuBar();
    	JMenu file = new JMenu("Datei");
    	JMenu about = new JMenu("About");
    	
    	JMenuItem showSrc = new JMenuItem("Show source folder");
    		showSrc.addActionListener(controller);
    		showSrc.setActionCommand("ShowSrc");
    	JMenuItem showRes = new JMenuItem("Show destination folder");
    		showRes.addActionListener(controller);
    		showRes.setActionCommand("ShowRes");
    	JMenuItem exit = new JMenuItem("Exit");
    		exit.addActionListener(controller);
    		exit.setActionCommand("Exit");
    	
    	JMenuItem info = new JMenuItem("Info");
    		info.addActionListener(controller);
    		info.setActionCommand("Info");
    	
    	file.add(showSrc);
    	file.add(showRes);
    	file.add(exit);
    	
    	about.add(info);
    	
    	menuLeiste.add(file);
    	menuLeiste.add(about);
    	
    	
    	
    	JPanel mainArea = new JPanel(new BorderLayout());
    	
    	mainArea.add(menuLeiste, BorderLayout.NORTH);
    	
    	//selectFolder things
    	JPanel selectFolders = new JPanel(new GridLayout(1,3));
    		JPanel selectSource = new JPanel(new FlowLayout());
    			JButton selectSrc = new JButton("Source: ");
    				selectSrc.addActionListener(controller);
    				selectSrc.setActionCommand("selectSrc");
    			selectSource.add(selectSrc);
    			//srcPath = new JTextField("C:\\Users\\Florian\\SkyDrive\\ImageEditor\\Projekt3\\ImageEditorGUI\\src\\source", 25);
    			srcPath = new JTextField(System.getProperty("user.home"),25);	
    				srcPath.addActionListener(controller);
    				srcPath.setActionCommand("srcTextFeld");
    			selectSource.add(srcPath);
    		JPanel selectResult = new JPanel(new FlowLayout());
    			JButton selectRes = new JButton("Destination: ");
    				selectRes.addActionListener(controller);
    				selectRes.setActionCommand("selectRes");
    			selectResult.add(selectRes);
    			//resPath = new JTextField("C:\\Users\\Florian\\SkyDrive\\ImageEditor\\Projekt3\\ImageEditorGUI\\src\\result", 25);
    			resPath = new JTextField(System.getProperty("user.home"),25);	
    				resPath.addActionListener(controller);
    				resPath.setActionCommand("resTextFeld");
    			selectResult.add(resPath);
    		
    		selectFolders.add(selectSource);
    		selectFolders.add(selectResult);
    		
    	mainArea.add(selectFolders, BorderLayout.CENTER);
    	
    	
    	
    	JPanel untenArea = new JPanel(new BorderLayout());
    	JSeparator line = new JSeparator();
    	untenArea.add(line, BorderLayout.NORTH);
    		JPanel settings = new JPanel(new GridLayout(1,2));
    			JPanel mode = new JPanel(new GridLayout(5,1));
    			
    			//Mode selection
    			JLabel modeTitle = new JLabel("Select mode:");
    			
    			JRadioButton mode1 = new JRadioButton("sum Image");//oev.ioservices.SumImageProcessingService
    				mode1.addActionListener(controller);
    				mode1.setActionCommand("oev.ioservices.SumImageProcessingService");
    			JRadioButton mode2 = new JRadioButton("Video");//Vid
    				mode2.addActionListener(controller);
    				mode2.setActionCommand("Vid");    			
     			JRadioButton mode4 = new JRadioButton("Special Video");//VidSpecial2Quad
     				mode4.addActionListener(controller);
     				mode4.setActionCommand("VidSpecial2Quad");
     			JRadioButton mode5 = new JRadioButton("Tunnel");
     				mode5.addActionListener(controller);
     				mode5.setActionCommand("Tunnel");
    			modeGroup = new ButtonGroup();    				
    			modeGroup.add(mode1);
    			modeGroup.add(mode2);
    			modeGroup.add(mode4);
    			modeGroup.add(mode5);
    			
    			mode.add(modeTitle);
    			mode.add(mode1);
    			mode.add(mode2);    		
    			mode.add(mode4);
    			mode.add(mode5);
    		settings.add(mode);
    		
    			//funktion selection
    			JPanel fkt = new JPanel(new GridLayout(4,1));
    			JLabel fktLabel = new JLabel("Function for brightness comparison:");
    			fkt.add(fktLabel);
    				String[] functions = {"1: physical", "2: near natural perception (recommended)", "3: luminance", "4: average (true long exposure)"};
    			fktBox = new JComboBox(functions);
    			fktBox.setSelectedIndex(1);    				
    			fkt.add(fktBox);
    			
    			//amount text fields
    			JPanel amount = new JPanel(new FlowLayout());
    				JLabel anzahl = new JLabel("amount input frames: ");
    				amount.add(anzahl);
    				amountFrames = new JTextField(4);    					
    				amount.add(amountFrames);
    				
    			fkt.add(new JLabel(""));
    			fkt.add(new JLabel(""));
    			
    			fkt.add(amount);
    			JPanel nachziehen = new JPanel(new FlowLayout());
    				JLabel nachziehenLabel = new JLabel("addition amount: ");
    				nachziehen.add(nachziehenLabel);
    				amountNachziehen = new JTextField(6);    					
    				nachziehen.add(amountNachziehen);
    			fkt.add(nachziehen);
    			
    			JPanel startFrame = new JPanel(new FlowLayout());
				JLabel startFrameLabel = new JLabel("start at frame Nr: ");
				startFrame.add(startFrameLabel);
				startFrameField = new JTextField("1",6);					
				startFrame.add(startFrameField);
			fkt.add(startFrame);
    		
			settings.add(fkt);
    		
    	untenArea.add(settings, BorderLayout.CENTER);
    	
    	//Read documentation Zeile
    	JPanel ganzUnten = new JPanel(new BorderLayout());
    		JPanel bilderPreview = new JPanel();
    		
    			    		
    			JLabel previewSpace = new JLabel("read documentation on http://www.tf-fotovideo.de/oev/");
				previewSpace.addMouseListener(new MouseListener() {
					
					@Override
					public void mouseReleased(MouseEvent arg0) {}
					
					@Override
					public void mousePressed(MouseEvent arg0) {}
					
					@Override
					public void mouseExited(MouseEvent arg0) {}	
					
					@Override
					public void mouseEntered(MouseEvent arg0) {}
										
					@Override
					public void mouseClicked(MouseEvent arg0) {
						
						try {
							Desktop.getDesktop().browse(new URI("http://www.tf-fotovideo.de/oev/"));
						} catch (IOException | URISyntaxException e) {							
							e.printStackTrace();
						}
						
					}
				});
				
    			
    			bilderPreview.add(previewSpace);
    		ganzUnten.add(bilderPreview, BorderLayout.NORTH);
    		
    		
    		//Trennlinie
    		JSeparator line2 = new JSeparator();
    		ganzUnten.add(line2, BorderLayout.CENTER);
    		
    		//start,stop,ladebalken area
    		JPanel startArea = new JPanel(new GridLayout(1,2));
    			JPanel progress = new JPanel();
    			
    			//Progress Bar
    			startArea.add(progress);
    				JPanel progressBarArea = new JPanel(new BorderLayout());
    					progressBar = new JProgressBar(0,59);
    					progressBar.setStringPainted(true);
    					progressBarArea.add(progressBar, BorderLayout.CENTER);
    				progress.add(progressBar);
    				
    				//Fortschrittstext
    				JPanel actions = new JPanel(new BorderLayout());
    				lastAction1 = new JLabel("Waiting for Start...");
    				actions.add(lastAction1, BorderLayout.NORTH);
    				lastAction2 = new JLabel("LA2");
    				actions.add(lastAction2, BorderLayout.CENTER);
    				lastAction3 = new JLabel("LA3");
    				actions.add(lastAction3, BorderLayout.SOUTH);
    				progress.add(actions);
    			startArea.add(progress);
    			
    			//Start Button
    			JButton start = new JButton("Start");
    				start.addActionListener(controller);
    				start.setActionCommand("Start");
    			startArea.add(start);
    			
    			//Stop Button
    			JButton stop = new JButton("Stop");
    				stop.addActionListener(controller);
    				stop.setActionCommand("Stop");
    			startArea.add(stop);
    		ganzUnten.add(startArea, BorderLayout.SOUTH);
    	
    		untenArea.add(ganzUnten, BorderLayout.SOUTH);
    	
    	mainArea.add(untenArea, BorderLayout.SOUTH);
    	
    	add(mainArea);
    	pack();
    	
    			
	}
	
	public void update(Observable o, Object obj){
		Model m = (Model) o;
		srcPath.setText(m.getSrcPath());
		resPath.setText(m.getResPath());
		lastAction1.setText(m.getLastAction1());
		lastAction2.setText(m.getLastAction2());
		lastAction3.setText(m.getLastAction3());
		progressBar.setMaximum(m.getMaxOperations());
		progressBar.setValue(m.getProgress());
	}
	
	public String getAmountFrames(){
		return amountFrames.getText();
	}
	
	public String getNachziehendeFrames(){
		return amountNachziehen.getText();
	}
	
	public String getStartFrame(){
		return startFrameField.getText();
	}
	
	public String getSrcPath(){
		return srcPath.getText();
	}
	
	public String getResPath(){
		return resPath.getText();
	}
	

	
	
}
